...
#'
#' This function takes a dataframe of trees, which is output from a BART model, and organises
...
#'
#'
#' @param treeData A dataframe that contains the tree structures generated by a BART model.
...
#'
#' @examples
#' # Assuming `bart_output` is a dataframe with BART model trees
#' trees_list <- treeList_new(bart_output)
#'
#' @export
#'
#'
...
treeList <- function(treeData, iter = NULL, treeNo = NULL) {


  # Error check for 'iter' input
  if (!is.null(iter)) {
    if (iter < 1 || iter > treeData$nMCMC) {
      stop("Error: 'iter' value is out of allowed range. Max iteration is ", treeData$nMCMC, ".")
    }
  }

  # Error check for 'treeNo' input
  if (!is.null(treeNo)) {
    if (treeNo < 1 || treeNo > treeData$nTree) {
      stop("Error: 'treeNo' value is out of allowed range. Max tree number is ", treeData$nTree, ".")
    }
  }
