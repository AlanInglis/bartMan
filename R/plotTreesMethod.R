...
plotTreesMethod <- function(model,
                      data,
                      response,
                      treeNum = 1,
                      iteration = 1,
  ...
  treeStructure <-  plotPackageTrees(model = model, treeNum, iteration)

  if(inherits(model, "wbart")){
    pTree <- treeStructure[[treeNum]]
  }else if(inherits(model, "bart")){
    pTree <- treeStructure
  }

  if(plotType == "dendrogram"){
  gp <- ggraph(pTree, "dendrogram", ) +
    geom_edge_elbow() +
    geom_node_label(aes(label = label, color = label)) +
    theme_graph() +
    theme(legend.position = "none")
  }else if(plotType == "icicle"){
   gp <-  ggraph(pTree, "partition") +
...
plotPackageTrees.bart <- function(model, treeNum = NULL, iteration = NULL) {
  dbartsTrees(model,treeNum,iteration)
}
...
dbartsTrees <- function(model, treeNum = NULL, iteration = NULL) {
  ...
  # treesAll <- ba$fit$getTrees()
  #
  # # NEED TO SPLIT BY SAMPLE AND CHAIN
  # treesSplit <- treesAll %>%
  #   group_split(cumsum(n == 1000), keep = FALSE)
  #
  # trees <- treesSplit[[treeNum]]
  # set up data for plot
    ...
    # get depth
    maxDepth <- dbarts:::getTreeDepthAndSize(trees)
    maxDepth <- maxDepth[1]
    ...
    if (maxDepth > 2) {
      ...
      # remove duplicated rows and single row entrie
